...
from search_algorithms import mdp_planning
...
if __name__ == "__main__":
    ...
    world_mdp = make_custom_scenario()
    path_mdp, nodes_mdp = mdp_search(world_mdp)

    print(f"MDP Planning --> path length: {len(path_mdp) if path_mdp else 'None'}, nodes expanded: {nodes_mdp}")

    world_mdp.render(path=path_mdp, title="MDP Planning (Risk-Aware)")
    world_mdp.simulate_path(
        path_mdp,
        search_fn=None,
        delay=0.4,
        algorithm_name="MDP Planning"
    )